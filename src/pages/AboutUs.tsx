    ...
    <div className="overflow-hidden">
        ...
        <SectionWrapper>
            <motion.div {...f} className="max-w-4xl mx-auto">
                <div className="card-glass gold-border rounded-2xl p-8 md:p-12">
                    <h3 className="text-xl font-bold gold-text mb-4">長翔的故事</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
                {advantages.map((a, i) => (
                    <motion.div key={a.title} {...f} transition={{ ...f.transition, delay: i * 0.1 }}
                        ...
                        <div className="w-12 h-12 rounded-full border border-[#D4AF37]/20 flex items-center justify-center mb-4 group-hover:border-[#D4AF37]/50 transition-all">
                            <a.icon size={24} className="text-[#D4AF37]" />
                        </div>
        ...
        <SectionWrapper title="產業鏈介紹" subtitle="涵蓋多元領域的專業團隊">
            <motion.div {...f} className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
                {industries.map(ind => (
                    <span key={ind} className="px-5 py-2.5 card-glass gold-border rounded-full text-sm text-white/80 hover:border-[#D4AF37]/40 hover:text-[#D4AF37] transition-all cursor-default">{ind}</span>
                ))}
            </motion.div>
        </SectionWrapper>
        ...
        <SectionWrapper className="text-center py-24">
            <motion.div {...f}>
                <h2 className="text-3xl font-black gold-text mb-6">成為長翔的一員</h2>
                <p className="text-gray-400 text-lg mb-8">如果您渴望找到一群真心互助的事業夥伴，歡迎親自來體驗</p>
                <a href="https://www.facebook.com/BNI.Chang.Siang.Chapter/" target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-10 py-4 brushed-gold text-[#0A1628] font-bold rounded-full hover:opacity-90 transition-opacity">
