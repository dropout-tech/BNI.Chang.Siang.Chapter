...
export type AdminAccessSource = 'bypass' | 'member_admin' | 'claimed_allowlist' | 'none';
...
export function getAdminAccessSource(user: AuthLikeUser | null | undefined, linkedMember: LinkedMemberAccount | null): AdminAccessSource {
    if (!user) return 'none';
    if (isBypassClaimAdminEmail(user.email)) return 'bypass';
    if (!linkedMember) return 'none';
    if (linkedMember.is_admin === true) return 'member_admin';
    if (isAdminEmail(user.email)) return 'claimed_allowlist';
    return 'none';
}
