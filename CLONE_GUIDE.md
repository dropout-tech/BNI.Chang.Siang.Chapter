...
本專案是 BNI 長翔名人堂白金分會的官方網站，採用以下技術棧：
...
| 問題 | 範例值（長翔分會） | 用途 |
|------|---------------------|------|
| 分會中文名稱？ | `長翔名人堂白金分會` | 品牌顯示 |
| 分會英文名稱？ | `ChangSiang` | URL、代碼 |
| 分會全名？ | `BNI 長翔名人堂白金分會` | SEO、結構化資料 |
| 分會英文全名？ | `BNI Chang Siang Chapter` | 法律文件 |
| 例會時間（星期幾、幾點到幾點）？ | `週三 06:30-08:30` | 結構化資料、FAQ |
| 例會形式（線上/實體/混合）？ | `線上` | SEO 關鍵字 |
| 分會所在城市？ | `台北市` | 地理 SEO |
| 網站網域？ | `changsiang.tw` | 部署、SEO |
| Facebook 粉專 URL？ | `https://www.facebook.com/BNIChangSiang` | 社群連結 |
| Instagram URL？ | `https://www.instagram.com/bnichangsiang` | 社群連結 |
| Threads URL？（可選） | `https://www.threads.net/@chhubni` | 社群連結 |
| 會員認領方式？ | 中文全名 + 行業別 | 會員綁定帳號用 |
...
修改每個路由的 title / description / keywords，將「長翔」替換為新分會名稱。
...
> 提示：用全域搜尋替換 `長翔` → `【新名稱】`，`changsiang.tw` → `【新網域】`

### 3.5 `public/sitemap.xml`

替換所有 `https://changsiang.tw` 為新網域。
...
| `changsiang` | `【新英文名小寫】` |
| `changsiang.tw` | `【新網域】` |
| `BNIChangSiang` | `【新FB帳號名】` |
| `bnichangsiang` | `【新IG帳號名】` |
| `chhubni` | `【新Threads帳號】` |
